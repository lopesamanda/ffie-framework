...
**Content:** e.g. Valentina/INSIDER, Tainá/A-EYE, John Bell/OPEN HUMAN — a deliberately varied trio (one Dominant Dystopian, one Feminist Preferred Future, one hegemonic-position entry) so the range of the matrix is visible in three examples, not just its extremes.
...
**What the user sees:** 3 real future cards with quadrant tags, a light-touch "these came from the same 18-card deck you're about to use" note, a continue action.
...
**UX interaction:** draw 2–3 cards via a weighted-random draw (not a checklist — the physical/tactile framing from both the Oracle and Thing From The Future precedents). The system immediately surfaces the specific tension generated by that combination. Then 1–2 short reflective prompts, answered in a sentence or two, e.g. *"Where do you feel this tension yourself — in your work, your community, your own use of AI?"*
...
**Narrative Cards involved:** the central mechanic of this entire stage — this is where the 18-card deck is actually used.
...
The full 18-card Narrative Card deck content (`docs/ffie_narrative_cards.md`, not yet extracted — only the 5 Risk cards are documented so far) is required before Reflection can generate real tension statements. This is the actual blocker for the next Cursor prompt, not a scope question.
